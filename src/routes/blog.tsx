...
export const Route = createFileRoute("/blog")({
  head: () => ({
    meta: [
      { title: "Blog — Rohit" },
      { name: "description", content: "Articles on design systems, AI, and modern web engineering." },
      { property: "og:title", content: "Blog — Rohit" },
      { property: "og:description", content: "Articles on design systems, AI, and modern web engineering." },
    ...
    <PageShell>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {posts.map((p) => (
            <a key={p.t} href="#" className="soft-card group flex flex-col rounded-3xl p-6 transition hover:-translate-y-1">
              ...
              <h2 className="mt-3 font-bold text-foreground transition group-hover:text-gradient">{p.t}</h2>
      ...
      <div className="mx-auto mt-24 max-w-4xl">
        ...
        <div className="soft-card relative rounded-3xl p-8 sm:p-12">
          ...
          <p className="text-lg leading-relaxed text-foreground/90 sm:text-xl">"{testimonials[i].q}"</p>
          <div className="mt-8 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button aria-label="Prev" onClick={() => setI((i - 1 + testimonials.length) % testimonials.length)} className="soft-card grid h-9 w-9 place-items-center rounded-lg hover:text-sky">
                <ChevronLeft className="h-4 w-4" />
              </button>
              <button aria-label="Next" onClick={() => setI((i + 1) % testimonials.length)} className="soft-card grid h-9 w-9 place-items-center rounded-lg hover:text-sky">
